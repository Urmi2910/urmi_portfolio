...
function PrerequisitesPanel({ copy }: { copy: PrerequisitesCopy }) {
  return (
    <div
      className="rounded-md border border-[#c7d2fe] bg-[#eef2ff] px-5 py-4 sm:px-8 sm:py-5"
      role="img"
    ...
    >
      <div className="flex items-center gap-2.5 border-b border-[#c7d2fe]/70 pb-3">
        <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#2563eb]">
          <Info className="h-3 w-3 text-white" strokeWidth={2.75} aria-hidden />
        </span>
        <h4 className="text-base font-semibold text-[#1e3a8a] sm:text-lg">{copy.title}</h4>
      </div>

      <ol className="mt-3 list-decimal space-y-2.5 pl-5 text-sm leading-snug text-[#374151] sm:space-y-3 sm:text-base sm:leading-relaxed">
        {copy.items.map((item) => (
